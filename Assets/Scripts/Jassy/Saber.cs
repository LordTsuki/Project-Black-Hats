...
public class Saber : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.K) && status.firstHit > 0 && status.secondHit <= 0) && (timeCooldownAttack1 == 0))
        {
            SecondAttack();
        }
        if ((Input.GetKeyDown(KeyCode.K) && status.secondHit > 0 && status.thirdHit <= 0) && (timeCooldownAttack2 == 0))
        {
            ThirdAttack();
        }
    }
